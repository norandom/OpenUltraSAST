...
1. Taxonomy spine: CWE-699 families aligned to our verifiers (recommended, ~10 families) or the mechanism vocabulary with CWE as an attribute?
2. Regression budget: the measured per-class seed-noise floor (recommended) or a fixed number?
3. Which model runs the detectors and which the meta-agent? Research supports a cheap meta-agent and spending on the detector.
4. Round-zero verifiers: which families get a sandbox recipe first, and do families without a verifier report at `suspicion` only until they have one?
5. Does `authorization-obligations` task 5 (the lever) become one family's evolve round, or stay a spec task?
